...
The property 'spring.cloud.stream.dynamicDestinations' can be used for restricting the dynamic destinations names to a set known beforehand (whitelisting).
If the property is not set, any destination can be bound dynamicaly.

The `BinderAwareChannelResolver` can be used directly as in the following example, in which a REST controller uses the request payloadresolves the dynamic destinations using the SpEL expression evaluated against the message payload.
...
For example, in a router using a SpEL expression based on the `target` field of an incoming message.
