...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.NONE, classes = ProgrammaticallyBoundChannelsWithDestinationsTests.TestProgrammaticBindings.class, properties = {
		"spring.cloud.stream.bindings.input.destination=testtock.0",
		"spring.cloud.stream.bindings.output.destination=testtock.1" })
public class ProgrammaticallyBoundChannelsWithDestinationsTests {
	...
	public void testSourceOutputChannelBound() {
		...
		Mockito.verify(binder).bindConsumer(eq("testtock.0"), anyString(), eq(input.getTarget()),
				Mockito.<ConsumerProperties>any());
		Mockito.verify(binder).bindProducer(eq("testtock.1"), eq(output.getTarget()),
