...
public class RawModeKafkaBinderTests extends KafkaBinderTests {
	...
	protected KafkaTestBinder getBinder() {
		...
		kafkaBinderProperties.setMode(KafkaMessageChannelBinder.Mode.raw);
		return super.getBinder(kafkaBinderProperties);
	}
	...
	public void testSendAndReceive() throws Exception {
		...
		Binding<MessageChannel> producerBinding = binder.bindProducer("foo.0", moduleOutputChannel, new ProducerProperties());
	...
	public void testSendAndReceiveWithExplicitConsumerGroup() {
		...
		Binding<MessageChannel> producerBinding = binder.bindProducer("baz.0", moduleOutputChannel, new ProducerProperties());
		Binding<MessageChannel> input1Binding = binder.bindConsumer("baz.0", "test", module1InputChannel, new ConsumerProperties());
		// A new module is using the tap as an input channel
		String fooTapName = "baz.0";
		Binding<MessageChannel> input2Binding = binder.bindConsumer(fooTapName, "tap1", module2InputChannel, new ConsumerProperties());
		// Another new module is using tap as an input channel
		String barTapName = "baz.0";
		Binding<MessageChannel> input3Binding = binder.bindConsumer(barTapName, "tap2", module3InputChannel, new ConsumerProperties());
		...
		input3Binding = binder.bindConsumer(barTapName, "tap2", module3InputChannel, new ConsumerProperties());
